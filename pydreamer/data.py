...
class EpisodeRepository(ABC):
    @abstractmethod
    def save_data(
        self, data: Dict[str, np.ndarray], episode_from: int, episode_to: int
    ):
        ...
...
class MlflowEpisodeRepository(EpisodeRepository):
    def __init__(self, artifact_uris: Union[str, List[str]]):
        super().__init__()
        self.artifact_uris = (
            [artifact_uris] if isinstance(artifact_uris, str) else artifact_uris
        )
        self.read_repos: List[ArtifactRepository] = [
            get_artifact_repository(uri) for uri in self.artifact_uris
        ]
    ...
    def save_data(
        ...
        fname = self.build_episode_name(
            episode_from, episode_to, reward, n_steps, chunk_seq=chunk_seq
        )
        print_once(
            f"Saving episode data ({chunk_seq}):",
            self.write_repo.artifact_uri + "/" + fname,
        )
...
class DataSequential(IterableDataset):
    ...
    def reload_files(self, is_first=False):
        ...
        if verbose:
            debug(
                f"Found total files|steps: {len(files_all)}|{steps_total}, filtered: {len(self.files)}|{steps_filtered}"
            )

    def should_reload_files(self):
        return self.reload_interval and (
            time.time() - self.last_reload > self.reload_interval
        )
    ...
    def iter_single(self, ix):
        ...
        for file in self.iter_shuffled_files():
            ...
            it = self.iter_file(
                file, self.batch_length, skip_random, first_shorter_length
            )
            ...
            if last_partial_batch is not None:
                for batch, partial in it:
                    assert (
                        not partial
                    ), "First batch must be full. Is episode_length < batch_size?"
    ...
    def iter_file(
        self, file: FileInfo, batch_length, skip_random=False, first_shorter_length=None
    ):
        try:
        ...
        if self.reset_interval:
            random_resets = self.randomize_resets(
                data["reset"], self.reset_interval, self.batch_length
            )
        else:
