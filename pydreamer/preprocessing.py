...
class Preprocessor:
    ...
    def apply(
        self, batch: Dict[str, np.ndarray], expandTB=False
    ) -> Dict[str, np.ndarray]:
        print_once(
            "Preprocess batch (before): ",
            {k: v.shape + (v.dtype.name,) for k, v in batch.items()},
        )
        ...
        if expandTB:
            batch = {
                k: v[np.newaxis, np.newaxis] for k, v in batch.items()
            }  # (*) => (T=1,B=1,*)
        ...
        batch["image"] = batch[
            self.image_key
        ]  # Use something else (e.g. map_masked) as image
        ...
        if "map_seen" in batch:
            ...
            batch["map_seen_mask"] = np.ones((T, B) + batch["map"].shape[-2:]).astype(
                int
            )
