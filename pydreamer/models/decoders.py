...
class MultiDecoder(nn.Module):
    def __init__(self, features_dim, config):
        ...
        self.terminal = DenseBernoulliDecoder(
            in_dim=features_dim,
            hidden_layers=config["terminal_decoder_layers"],
            layer_norm=config["layer_norm"],
        )
        self.vecobs = DenseNormalDecoder(
            in_dim=features_dim,
            out_dim=64,
            hidden_layers=4,
            layer_norm=config["layer_norm"],
    ...
    def training_step(
        self, features: TensorTBIF, obs: Dict[str, Tensor], extra_metrics: bool = False
    ) -> Tuple[TensorTBI, Dict[str, Tensor], Dict[str, Tensor]]:
        tensors = {}
        metrics = {}

        loss_image_tbi, loss_image, image_rec = self.image.training_step(
            features, obs["image"]
        )
        metrics.update(loss_image=loss_image.detach().mean())
        tensors.update(loss_image=loss_image.detach(), image_rec=image_rec.detach())

        loss_vecobs_tbi, loss_vecobs, vecobs_rec = self.vecobs.training_step(
            features, obs["vecobs"]
        )
        ...
        loss_terminal_tbi, loss_terminal, terminal_rec = self.terminal.training_step(
            features, obs["terminal"]
        )
        metrics.update(loss_terminal=loss_terminal.detach().mean())
        tensors.update(
            loss_terminal=loss_terminal.detach(), terminal_rec=terminal_rec.detach()
        )

        assert (
            loss_image_tbi.shape
            == loss_vecobs_tbi.shape
            == loss_reward_tbi.shape
            == loss_terminal_tbi.shape
        )
        loss_reconstr = (
            self.image_weight * loss_image_tbi
        ...
        if extra_metrics:
            ...
            loss_rewardp = (
                loss_reward * mask_rewardp / mask_rewardp
            )  # set to nan where ~mask
            ...
            loss_rewardn = (
                loss_reward * mask_rewardn / mask_rewardn
            )  # set to nan where ~mask
            ...
            loss_terminal1 = (
                loss_terminal * mask_terminal1 / mask_terminal1
            )  # set to nan where ~mask
...
class ConvDecoder(nn.Module):
    ...
    def training_step(
        self, features: TensorTBIF, target: TensorTBCHW
    ) -> Tuple[TensorTBI, TensorTB, TensorTBCHW]:
        assert len(features.shape) == 4 and len(target.shape) == 5
        I = features.shape[2]
        target = insert_dim(
            target, 2, I
        )  # Expand target with iwae_samples dim, because features have it
...
class CatImageDecoder(nn.Module):
    ...
    def loss(self, output: Tensor, target: Tensor) -> Tensor:
        ...
        output, bd = flatten_batch(
            output, len(self.out_shape)
        )  # (*,C,H,W) => (B,C,H,W)
        target, _ = flatten_batch(target, len(self.out_shape) - 1)  # (*,H,W) => (B,H,W)

        if self.min_prob == 0:
            loss = F.nll_loss(
                F.log_softmax(output, 1), target, reduction="none"
            )  # = F.cross_entropy()
        else:
            prob = F.softmax(output, 1)
            prob = (1.0 - self.min_prob) * prob + self.min_prob * (
                1.0 / prob.size(1)
            )  # mix with uniform prob
...
class DenseBernoulliDecoder(nn.Module):
    ...
    def training_step(
        self, features: TensorTBIF, target: Tensor
    ) -> Tuple[TensorTBI, TensorTB, TensorTB]:
...
class DenseNormalDecoder(nn.Module):
    ...
    def loss(self, output: D.Distribution, target: Tensor) -> Tensor:
        var = (
            self.std ** 2
        )  # var cancels denominator, which makes loss = 0.5 (target-output)^2
        return -output.log_prob(target) * var

    def training_step(
        self, features: TensorTBIF, target: Tensor
    ) -> Tuple[TensorTBI, TensorTB, Tensor]:
        assert len(features.shape) == 4
        I = features.shape[2]
        target = insert_dim(
            target, 2, I
