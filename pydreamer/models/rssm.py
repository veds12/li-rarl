        ...
        for i in range(T):
            if not do_open_loop:
                post, (h, z) = self.cell.forward(
                    embeds[i], actions[i], reset_masks[i], (h, z)
                )
            else:
                post, (h, z) = self.cell.forward_prior(
                    actions[i], reset_masks[i], (h, z)
                )  # open loop: post=prior
            posts.append(post)
            states_h.append(h)
            samples.append(z)
...
class RSSMCell(nn.Module):
    def __init__(
        ...
        )  # No bias, because outputs are added
    ...
    def init_state(self, batch_size):
        ...
        return (
            ...
            torch.zeros(
                (batch_size, self.stoch_dim * (self.stoch_discrete or 1)), device=device
            ),
        )
    ...
    def zdistr(self, pp: Tensor) -> D.Distribution:
        ...
        if self.stoch_discrete:
            ...
            distr = D.OneHotCategoricalStraightThrough(
                logits=logits.float()
            )  # NOTE: .float() needed to force float32 on AMP
            distr = D.independent.Independent(
                distr, 1
